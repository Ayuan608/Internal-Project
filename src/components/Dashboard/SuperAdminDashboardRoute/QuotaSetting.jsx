...
const QuotaSetting = () => {
    ...
    if (Array.isArray(arr)) {
      arr.forEach((quota) => {
        ...
        if (formatted[deptKey]) {
          formatted[deptKey] = {
            morning:
              quota?.shiftQuota?.morning !== undefined &&
              quota?.shiftQuota?.morning !== null
                ? String(quota.shiftQuota.morning)
                : "",
            night:
              quota?.shiftQuota?.night !== undefined &&
              quota?.shiftQuota?.night !== null
  ...
  const handleSaveChanges = async (dept) => {
    ...
    if (morning === "" || night === "") {
      alert(t("Please enter both morning and night quotas"));
      return;
    }
    ...
    try {
      ...
      alert(t("Failed to save quota. Try again."));
    } finally {
      setSavingDept(null);
    }
  };
  ...
  const handleResetAll = async () => {
    ...
    try {
      ...
      alert(t("Failed to reset quotas. Try again."));
    }
  };

  const departments = [
    { key: "csr", name: t("CSR Department") },
    { key: "deposit", name: t("Deposit Department") },
    { key: "withdrawal", name: t("Withdrawal Department") },
  ];
  ...
  const SuccessMessage = ({ dept }) => (
    <div className="fixed top-4 right-4 bg-green-600 text-white px-4 py-3 rounded-lg shadow-lg flex items-center gap-3 animate-in slide-in-from-top duration-300 z-50">
      ...
      <span className="font-medium">
        {dept === "all"
          ? t("All quotas reset successfully!")
          : `${departments.find((d) => d.key === dept)?.name} ${t(
              "saved successfully!"
            )}`}
      </span>
    </div>
    ...
    <div className="min-h-screen p-2">
      ...
      <div className="m-4">
        ...
        <p className="text-gray-400">
          {t("Configure quota targets for each department and shift")}
        </p>
      </div>
            ...
            <div className="p-6 space-y-5">
              <div>
                ...
                <input
                  ...
                  min="0"
                  placeholder={t("Enter morning quota")}
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all outline-none text-white font-medium bg-[#1a1b1e]"
                  min="0"
                  placeholder={t("Enter night quota")}
                />
              </div>
              ...
              >
                {savingDept === key ? t("Saving...") : t("Save Changes")}
              </button>
            </div>
          </div>
        ))}
